...
Phen_imp::Phen_imp(string n,Etat*e, Serveur*s, double a, double b, long int c, long int d, long int f, long int g, long int h, double min, double max,  double u, double o):v_low(a),v_high(b),t_del(c),t_rise(d),pwidth(f),t_fall(g),period(h),sat_min(min),sat_max(max),mu(u),sigma(o){
