...
int main(){
	...
	const double VAL_IMP_SAT_MIN = 0.0;
	const double VAL_IMP_SAT_MAX = 14.0;
	...
	Etat etat_ph("Etat pH",INIT_PH,I_PHEN_PH,I_CTRL_PH,&serv);
	...
	// Run the simulation
	serv.log("\nStarting simulation\n");
	Simulator mySimulator(NB_TICKS, TIME_UNIT);
	mySimulator.push_back(&phen_temp);
	mySimulator.push_back(&ctrl_temp);
	mySimulator.push_back(&etat_temp);
	mySimulator.push_back(&phen_ph);
	mySimulator.push_back(&ctrl_ph);
	mySimulator.push_back(&etat_ph);
	mySimulator.push_back(&serv);
